...
const Shuffle = ({
  ...
  useGSAP(
    () => {
      ...
      const removeHover = () => {
        const target = ref.current ? (ref.current as any).closest('button, a') || ref.current : null;
        if (hoverHandlerRef.current && target) {
          target.removeEventListener('mouseenter', hoverHandlerRef.current);
      ...
      const armHover = () => {
        ...
        const target = (ref.current as any).closest('button, a') || ref.current;
        target.addEventListener('mouseenter', handler);
      };

      // Always arm hover immediately so it works even if scroll trigger fails
      armHover();
