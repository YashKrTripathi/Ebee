...
const markersData: CityMarker[] = [
  { name: "Delhi", slots: "18 Free", coords: [28.6139, 77.2090] },
  { name: "Lucknow", slots: "12 Free", coords: [26.8467, 80.9462] },
  { name: "Guwahati", slots: "8 Free", coords: [26.1445, 91.7362] },
  { name: "Mumbai/Pune", slots: "24 Free", coords: [19.0760, 72.8777] },
  { name: "Hyderabad/Chennai", slots: "22 Free", coords: [15.5000, 79.5000] } // Positioned beautifully between Hyderabad and Chennai
];

export function NetworkMapSection({ onViewMap }: NetworkMapSectionProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch("/world_countries.json").then((res) => {
      ...
      .then(([worldData, indiaData]) => {
          ...
          onEachFeature: (feature, layer) => {
            ...
            layer.on({
              mouseover: (e) => {
                const l = e.target;
                l.setStyle({
                  fillColor: "#F5D77F", // slightly highlighted yellow
                  weight: 2
                });
              },
              mouseout: (e) => {
        ...
        markersData.forEach((marker) => {
          ...
          const customIcon = L.divIcon({
            html: markerHtml,
            className: "", // clears default styles
            iconSize: [100, 60],
            iconAnchor: [50, 20]
          });
  ...
  return (
    <section 
      id="charge-network-map" 
      className="relative overflow-hidden bg-gradient-to-br from-[#FFF8E1] to-[#FCE38A] py-16 sm:py-24 lg:py-28"
    >
        ...
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 items-center">
          ...
          <div className="lg:col-span-5 space-y-6 sm:space-y-8 text-left max-w-2xl mx-auto lg:mx-0 lg:max-w-none">
            <div className="space-y-4 sm:space-y-5">
              ...
              <div className="inline-flex items-center gap-2 rounded-full border border-[#E5A100]/20 bg-white/70 px-4 py-2 backdrop-blur-md w-fit shadow-sm">
            ...
            <div className="pt-2">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="group relative inline-flex items-center justify-center rounded-2xl bg-[#FFD60A] px-8 py-4.5 text-xs font-black uppercase tracking-[0.2em] text-neutral-950 shadow-[0_12px_28px_-8px_rgba(255,214,10,0.6)] transition-all duration-300 hover:bg-[#E8C509] hover:shadow-[0_16px_35px_-8px_rgba(255,214,10,0.8)] border border-[#FFE152]"
              >
          ...
          <div className="lg:col-span-7 flex justify-center items-center w-full">
            <motion.div
              ...
              className="relative w-full aspect-[4/3] min-h-[350px] sm:min-h-[420px] rounded-[24px] border border-white/60 bg-white/40 p-4 shadow-[0_32px_80px_-20px_rgba(0,0,0,0.08)] backdrop-blur-xl"
            >
              {/* Inner card padding wrapper */}
              <div className="relative w-full h-full overflow-hidden rounded-[20px] border border-neutral-200/50 bg-[#E8ECEF] shadow-inner">
                
                {/* The Map Div */}
                <div ref={mapContainerRef} className="w-full h-full z-0" />

                {/* Floating Bottom-Left Info Card */}
                <div className="absolute bottom-5 left-5 z-[1000] flex items-center gap-3 rounded-[18px] border border-neutral-100/40 bg-white/95 p-3.5 shadow-[0_12px_32px_rgba(0,0,0,0.08)] backdrop-blur-xl pointer-events-auto">
                  {/* Yellow Pin Icon Box */}
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#FFF9DB] text-[#FFD60A]">
