...
function ArticleCover({
  ...
  return <div className="relative min-h-[260px] overflow-hidden -[32px] border border-white/70 bg-neutral-100 shadow-[0_30px_80px_-30px_rgba(0,0,0,0.25)]">
      ...
      <div className="absolute left-6 top-6 border border-white/70 bg-white/80 px-4 py-2 text-[10px] font-black uppercase tracking-wider text-neutral-700 backdrop-blur-xl">
        {post.category}
      </div>
      <div className="absolute bottom-6 left-6 right-6 flex items-end justify-between gap-4">
        ...
        <div className="flex h-14 w-14 shrink-0 items-center justify-center bg-primary-500 text-neutral-950 shadow-lg">
...
function ContentBlock({
  ...
  if (block.type === "heading") {
    return <h2 className="pt-5 text-2xl md:text-3xl font-extrabold tracking-tight text-neutral-900">
        {block.text}
      </h2>;
  }
  if (block.type === "bullets") {
    return <ul className="space-y-3">
        {block.items.map(item => <li key={item} className="flex gap-3 text-[15px] leading-8 text-neutral-600">
...
export default function BlogDetail() {
  ...
  const relatedPosts = useMemo(() => {
    ...
    return blogs.filter(blog => blog.slug !== post.slug).slice(0, 3);
  }, [post]);
  useEffect(() => {
    if (!post) {
      ...
      <main className="flex-1">
        <section className="relative overflow-hidden pt-14 pb-14 lg:pt-20 lg:pb-20 border-b border-neutral-300 bg-transparent">
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            duration: 0.5
              ...
              <div className="inline-flex bg-primary-100 px-4 py-2 text-[11px] font-black uppercase tracking-wider text-primary-600">
                {post.category}
              </div>
        ...
        <article className="py-16 bg-white border-b border-neutral-200/50">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="space-y-6">
              {post.content.map((block, index) => <ContentBlock key={`${block.type}-${index}`} block={block} />)}
            </div>

            <div className="mt-12 border border-neutral-200/60 bg-neutral-50 p-6">
              <p className="text-[11px] font-black uppercase tracking-wider text-primary-600">
                Author
              </p>
            ...
            <div className="mt-8 flex flex-wrap gap-2">
              {post.tags.map(tag => <span key={tag} className="border border-neutral-200 bg-neutral-50 px-3 py-1.5 text-[10px] font-bold text-neutral-500">
                  {tag}
                </span>)}
            </div>
          </div>
        ...
        <section className="py-20 bg-neutral-50 border-b border-neutral-200/50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mb-10 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              ...
              <Link to="/blog" className="inline-flex items-center gap-2 text-[12px] font-black uppercase tracking-wider text-primary-600">
                View All
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {relatedPosts.map((related, index) => <motion.article key={related.slug} initial={{
              opacity: 0,
              y: 18
            }} whileInView={{
              opacity: 1,
              ...
              once: true
            }} className="group border border-neutral-200/60 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-primary-300 hover:shadow-xl hover:shadow-neutral-300/30">
                  <span className="bg-neutral-100 px-3 py-1.5 text-[10px] font-black uppercase tracking-wider text-primary-600">
                    {related.category}
                  </span>
                  ...
                  <Link to={`/blog/${related.slug}`} className="mt-6 inline-flex items-center gap-2 text-[12px] font-black uppercase tracking-wider text-primary-600">
                    Read More
        ...
        <section className="py-16 bg-gradient-to-r from-primary-500 via-primary-400 to-primary-500 border-b border-primary-600/30 font-body relative overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
            ...
            <motion.button type="button" onClick={() => setIsAuditModalOpen(true)} whileHover={{
            scale: 1.05,
            y: -2
          }} whileTap={{
            scale: 0.95
